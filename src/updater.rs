...
pub fn expected_asset_name(target: &str) -> String {
    expected_asset_name_for_binary("vibe-usage", target)
}

#[cfg(test)]
pub fn expected_asset_name_for_binary(binary_name: &str, target: &str) -> String {
    vibe_usage_updater::expected_asset_name(binary_name, target)
}

fn update_config() -> vibe_usage_updater::UpdateConfig {
    vibe_usage_updater::UpdateConfig::current("vibe-usage", CURRENT_VERSION)
}

pub fn run_update<F: FnMut(&str)>(mut log: F) -> Result<UpdateOutcome, String> {
    let config = update_config();
    match vibe_usage_updater::check_and_install(&config, |message| log(message))? {
        vibe_usage_updater::InstallOutcome::AlreadyLatest { current, latest } => {
            Ok(UpdateOutcome::AlreadyLatest { current, latest })
        }
        vibe_usage_updater::InstallOutcome::Updated(update) => {
            log(&format!(
                "Update applied. Restarting v{} ...",
                update.latest
            ));
            let args: Vec<OsString> = std::env::args_os().collect();
            vibe_usage_updater::exec_path(&update.executable, &args)?;
...
mod tests {
    ...
    fn asset_name_can_target_server_binary() {
        assert_eq!(
            expected_asset_name_for_binary("vibe-usage-server", "x86_64-unknown-linux-gnu"),
    ...
    fn semver_compare_handles_basic_cases() {
        assert!(vibe_usage_updater::is_newer("1.5.9", "1.5.8"));
        assert!(vibe_usage_updater::is_newer("1.6.0", "1.5.99"));
        assert!(vibe_usage_updater::is_newer("2.0.0", "1.99.99"));
        assert!(!vibe_usage_updater::is_newer("1.5.8", "1.5.8"));
        assert!(!vibe_usage_updater::is_newer("1.5.7", "1.5.8"));
    }

    #[test]
    fn semver_compare_tolerates_v_prefix_stripping() {
        assert!(vibe_usage_updater::is_newer("1.5.8", "1.5.7"));
    }
}
