...
After a full snapshot finalizes, the client invalidates its pull cursor and integrity receipt. The next pull clears the local remote cache and rebuilds it from sequence zero so records deleted by the server cannot remain visible locally.
