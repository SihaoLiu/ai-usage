...
Each reconciliation commits the immutable cache generation captured when that cycle began. If live source ingestion advances the cache while requests are in flight, the completed receipt remains tied to the captured generation and the next cycle sends the new delta. Records requested by the server are still checked against their captured fingerprints before upload, so a mutation cannot silently change the contents of the snapshot being finalized.
...
After a full local snapshot finalizes, the client invalidates its integrity receipt so a later cycle cannot reuse an owner report that predates the finalized snapshot. The peer-only pull cursor and remote cache remain valid because pull excludes the local machine; local snapshot deletions therefore cannot remove or change any record represented by that cursor. Server identity changes, pull-scope changes, explicit cache cleaning, and a failed peer integrity check when no complete pull has finished in the previous six hours rebuild peer-owned remote state from sequence zero.
...
An integrity mismatch remains visible as `Integrity Failed`. The client repairs a mismatch by clearing and refetching peer-owned state unless it has a complete pull for the current server and peer-only vendor scope within the previous six hours. Missing, invalid, future, stale, mismatched, and still-in-progress pull receipts cannot suppress repair. A mismatch immediately after a current complete pull does not repeat the same sequence-zero fetch; ordinary incremental pulls continue, and another full repair becomes eligible after the interval. Full-pull ownership is persisted with the page cursor, so a network error, process restart, or manual integrity check resumes the same pull and records completion only after its final page. If repair discovers an older server with a reduced vendor set, it keeps the negotiated cache and cursor, leaves the mismatch visible, and does not run another integrity check against the known-incomplete data. Later attempts probe for a server upgrade without discarding the accepted subset. The interval is measured between UTC instants, so daylight-saving transitions do not shorten or extend it.
...
The reverse skew is the one to keep short: a client still on the previous release cannot pull the new vendor's records, so its integrity verification of hosts that already upload them fails. Current clients keep the failure visible but pace sequence-zero repair pulls to at most one every six hours. Older clients may clear and refetch on every cycle. This resolves as soon as that machine upgrades (monitor mode's `update` command or `--auto-update` handles this).
